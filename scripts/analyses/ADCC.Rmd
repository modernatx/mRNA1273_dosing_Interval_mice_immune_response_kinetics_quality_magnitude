...
emmeans_plot <- contrast(
  ...
  scale_y_discrete(labels = ~ str_remove_all(.x, "dosing_interval_weeks") %>%
                     str_replace("-", "/") %>%
...
```{r, echo = FALSE}
ggsave(emmeans_plot, filename = "figures_tables/ADCC/ADCC_tests.pdf", width = 8, height = 4, device = cairo_pdf)
```
